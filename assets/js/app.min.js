...
d&&(a=window.innerHeight,d=d.offsetHeight,h[0]&&720<window.screen.availWidth&&(h[0].style.minHeight=`${Math.round(a-d)}px`));m(".btn-register",g=>{const p=g.dlg.querySelector(".notification-placeholder"),c=g.dlg.querySelector("form[name=registration]");c.addEventListener("submit",k=>k.preventDefault());const t=c.querySelector("input[type=submit]"),r=c.querySelector("input[name=registrationType][value=Individual]"),w=c.querySelector("input[name=registrationType][value=Team]"),A=c.querySelector(".registrationTeam"),
...
x);let H=document.createElement("div");var e=document.createElement("input");e.classList.add("form-control","form-control-2pbb");e.id=`teamMemberName.${k}`;e.name=`teamMemberName.${k}`;e.type="text";e.placeholder="Enter team member name";H.appendChild(e);e=document.createElement("div");var f=document.createElement("input");f.classList.add("form-control","form-control-2pbb");f.id=`teamMemberEmail.${k}`;f.name=`teamMemberEmail.${k}`;f.type="text";f.placeholder="Enter team member e-mail address";e.appendChild(f);
f=document.createElement("div");let v=document.createElement("button");v.classList.add("btn","btn-sm","btn-remove-entry");v.id=`teamMemberDel.${k}`;v.name=`teamMemberDel.${k}`;v.setAttribute("data-id",x);v.innerHTML='<span data-icon="&#xe005;" class="ic-mr-5"></span> Remove';f.appendChild(v);q.appendChild(H);q.appendChild(e);q.appendChild(f);u.appendChild(q);u.querySelectorAll("button.btn-remove-entry").forEach(I=>{I.addEventListener("click",y=>{y=I.getAttribute("data-id");y=u.querySelector(`.teamMember[data-id="${y}"]`);
y.parentNode.removeChild(y)})})}});const l={type:null,name:{first:null,last:null},email:null,phone:null,joinTeam:0,team:{name:null,members:[]},challenge:0,termsConditions:0};t.addEventListener("click",k=>{try{l.type=r.checked?1:2;if(0===C.value.length||0===D.value.length||0===z.value.length||0===E.value.length)throw"Fields marked with '*' are required. Please make sure all are filled.";if(z.value.match(b))l.name.first=C.value.trim(),l.name.last=D.value.trim(),l.email=z.value.trim(),l.phone=E.value.trim();
else throw"E-mail address is invalid. Please use a different one.";if(1===l.type)l.joinTeam=J.checked?1:0;else if(2===l.type){if(0===F.value.length)throw"Team name is mandatory. Please type in your team name.";l.team.name=F.value.trim();const e=u.querySelectorAll(".teamMember");if(0===e.length)throw"You haven't added any team members. Please add at least a team member or switch to individual registration.";e.forEach((f,x)=>{if(f.getAttribute("data-id")){const q=f.querySelector(`input[name="teamMemberName.${x}"]`).value.trim();
f=f.querySelector(`input[name="teamMemberEmail.${x}"]`).value.trim();if(0===q.length||0===f.length)throw"Team member name and e-mail address are mandatory.";if(f.match(b))l.team.members.push({name:q,email:f});else throw`Team member e-mail address for <strong>${q}</strong> is invalid. Please use a different one.`;}})}l.challenge=K.checked?1:L.checked?2:M.checked?3:0;if(0===l.challenge)throw"You haven't chosen the hackathon challenge.";l.termsConditions=N.checked?1:0;if(0===l.termsConditions)throw"Terms and Conditions must be agreed in order to complete hackathon registration.";
app.api("https://api.hackathon.is/registration/","POST",{action:"registration",value:l}).then(e=>e.json()).then(e=>{if(e.error)throw e.message;g.dlg.close();n(p,e.message,"success",10,!0)}).catch(e=>{n(p,e,"error",10,!0)})}catch(e){n(p,e,"error",10,!0)}})})}}}})();(()=>{app.render();app.menu()})();
