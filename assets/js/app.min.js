/*
 (c) 2023 IA?I AI. All rights reserved.
...
const app=(()=>{function m(b,a=null){b=document.querySelectorAll(b);0<b.length&&b.forEach(d=>{let h=d.getAttribute("data-dlg"),f=document.querySelector(h);h&&f&&d.addEventListener("click",()=>{Dialog.closeAll();const p=new Dialog(f.innerHTML,{dialogClassName:"modal-dialog",dialogPlaceholderClassName:"dialog-placeholder",closeOnEsc:!0,closeOnOutsideClick:!1,callback:{onShow:()=>{document.body.classList.add("no-scroll")},onClose:()=>{document.body.classList.remove("no-scroll")}}});p.dlg.querySelectorAll("label[for]").forEach(t=>
{var r=t.htmlFor;let w=p.dlg.querySelector(`#${r}`);r&&w&&(r=`elem${Math.random().toString(16).slice(2)}`,w.id=r,t.htmlFor=r)});a&&a(p,d.dataset||null);const c=p.dlg.querySelector(".close-dialog");c&&c.addEventListener("click",t=>{t.preventDefault();t.stopPropagation();p.close()})})})}function n(b,a,d="error",h=0,f=!1){if(b){const p=b.querySelectorAll(".notification"),c=document.createElement("p");c.classList.add("notification","notification-sm");c.classList.remove("error","success","info","warning");
c.classList.add(d);c.innerHTML=a;f?(c.classList.add("notification-floating","notification-animation"),0<p.length&&(c.style.top=`${45*p.length+20}px`)):0<p.length&&(b.innerHTML="");b.appendChild(c);0<h&&setTimeout(()=>{c.classList.remove("notification","notification-sm","notification-inline","notification-animation","notification-floating","error","success","info","warning");c.innerHTML="";c.parentNode.removeChild(c)},1E3*h)}}return{spin:'<div class="spinner spinner-sm"></div>',api:async function(b,
a="GET",d=""){d={method:a,mode:"cors",cache:"no-cache",credentials:"same-origin",headers:{Accept:"application/json","Content-Type":"application/json"},body:d?JSON.stringify(d):""};"GET"!==a&&"HEAD"!==a||delete d.body;return await fetch(b,d)},route:function(){const b=["route","method","query"];var a=window.location.pathname;const d=window.location.hash,h={};if(a&&(a=a.replace(/^\/?|\/?$/g,"").split("/"),0<a.length))for(let f=0;f<b.length;f++)h[b[f]]=a[f];d&&(h.hash=d.substring(1));return h},menu:function(){let b=
document.querySelector(".btn-menu"),a=document.querySelector(".slide-menu");null!==b&&null!==a&&b.addEventListener("click",d=>{d.preventDefault();d.stopPropagation();const h=new Dialog(`<div class="container">${a.innerHTML}</div>`,{dialogClassName:"slide-menu-dialog",dialogPlaceholderClassName:"dialog-placeholder",position:{x:0,y:0},callback:{onShow:()=>{b.style.visibility="hidden";document.body.classList.add("no-scroll");document.querySelector(".logo").classList.add("logo-slide-menu")},onClose:()=>
{b.style.visibility="visible";document.body.classList.remove("no-scroll");document.querySelector(".logo").classList.remove("logo-slide-menu")}}});(d=h.dlg.querySelector("button.btn-slide-close"))&&d.addEventListener("click",f=>{f.preventDefault();f.stopPropagation();h.close()})})},render:function(){const b=/^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/;var a=document.querySelector(".page");if(a){var d=a.querySelector("header");const h=a.querySelectorAll("section.events");a.querySelectorAll(".btn-register");
d&&(a=window.innerHeight,d=d.offsetHeight,h[0]&&720<window.screen.availWidth&&(h[0].style.minHeight=`${Math.round(a-d)}px`));m(".btn-register",f=>{const p=f.dlg.querySelector(".notification-placeholder"),c=f.dlg.querySelector("form[name=registration]");c.addEventListener("submit",k=>k.preventDefault());const t=c.querySelector("input[type=submit]"),r=c.querySelector("input[name=registrationType][value=Individual]"),w=c.querySelector("input[name=registrationType][value=Team]"),A=c.querySelector(".registrationTeam"),
B=c.querySelector(".agreeJoinTeam"),C=c.querySelector("input[name=firstName]"),D=c.querySelector("input[name=lastName]"),z=c.querySelector("input[name=email]"),E=c.querySelector("input[name=phone]"),J=c.querySelector("input[name=agreePartOfTeam]"),F=c.querySelector("input[name=teamName]"),u=c.querySelector(".team-container"),G=c.querySelector("button[name=teamMemberAdd]"),K=c.querySelector("input#challenge1"),L=c.querySelector("input#challenge2"),M=c.querySelector("input#challenge3"),N=c.querySelector("input[name=agreeTerms]");
r&&r.addEventListener("click",k=>{A.classList.add("hide");B.classList.remove("hide")});w&&w.addEventListener("click",k=>{A.classList.remove("hide");B.classList.add("hide")});u&&G&&G.addEventListener("click",k=>{k.preventDefault();k.stopPropagation();k=u.querySelectorAll(".teamMember").length;if(6>k){let x=Date.now().toString();document.createDocumentFragment();let q=document.createElement("fieldset");q.classList.add("teamMember","form-group","form-group-inline","cols-5-5-2");q.setAttribute("data-id",
x);let H=document.createElement("div");var e=document.createElement("input");e.classList.add("form-control","form-control-2pbb");e.id=`teamMemberName.${k}`;e.name=`teamMemberName.${k}`;e.type="text";e.placeholder="Enter team member name";H.appendChild(e);e=document.createElement("div");var g=document.createElement("input");g.classList.add("form-control","form-control-2pbb");g.id=`teamMemberEmail.${k}`;g.name=`teamMemberEmail.${k}`;g.type="text";g.placeholder="Enter team member e-mail address";e.appendChild(g);
g=document.createElement("div");let v=document.createElement("button");v.classList.add("btn","btn-sm","btn-remove-entry");v.id=`teamMemberDel.${k}`;v.name=`teamMemberDel.${k}`;v.setAttribute("data-id",x);v.innerHTML='<span data-icon="&#xe005;" class="ic-mr-5"></span> Remove';g.appendChild(v);q.appendChild(H);q.appendChild(e);q.appendChild(g);u.appendChild(q);u.querySelectorAll("button.btn-remove-entry").forEach(I=>{I.addEventListener("click",y=>{y=I.getAttribute("data-id");y=u.querySelector(`.teamMember[data-id="${y}"]`);
y.parentNode.removeChild(y)})})}});const l={type:null,name:{first:null,last:null},email:null,phone:null,joinTeam:0,team:{name:null,members:[]},challenge:0,termsConditions:0};t.addEventListener("click",k=>{try{l.type=r.checked?1:2;if(0===C.value.length||0===D.value.length||0===z.value.length||0===E.value.length)throw"Please make sure all required fields are filled in.";if(z.value.match(b))l.name.first=C.value.trim(),l.name.last=D.value.trim(),l.email=z.value.trim(),l.phone=E.value.trim();else throw"E-mail address is invalid. Please use a different one.";
if(1===l.type)l.joinTeam=J.checked?1:0;else if(2===l.type){if(0===F.value.length)throw"Please make sure you provide a name for your team.";l.team.name=F.value.trim();const e=u.querySelectorAll(".teamMember");if(0===e.length)throw"You haven't added any team members. Please add at least a team member or switch to individual registration.";e.forEach((g,x)=>{if(g.getAttribute("data-id")){const q=g.querySelector(`input[name="teamMemberName.${x}"]`).value.trim();g=g.querySelector(`input[name="teamMemberEmail.${x}"]`).value.trim();
if(0===q.length||0===g.length)throw"Team member name and e-mail address are mandatory.";if(g.match(b))l.team.members.push({name:q,email:g});else throw`Team member e-mail address for <strong>${q}</strong> is invalid. Please use a different one.`;}})}l.challenge=K.checked?1:L.checked?2:M.checked?3:0;if(0===l.challenge)throw"You haven't chosen the hackathon challenge.";l.termsConditions=N.checked?1:0;if(0===l.termsConditions)throw"Terms and Conditions must be agreed in order to complete hackathon registration.";
app.api("https://api.hackathon.is/api/registration/","POST",{action:"registration",value:l}).then(e=>e.json()).then(e=>{if(e.error)throw e.message;f.dlg.close();f.dlg.innerHTML=`<section class="registration-successful text-center"><span data-icon="&#xe001;" class="ic-colored ic-x3" style="--color:#28a745;"></span><h3>${e.message}</h3></section>`}).catch(e=>{n(p,e,"error",10,!0)})}catch(e){n(p,e,"error",10,!0)}})})}}}})();(()=>{app.render();app.menu()})();
