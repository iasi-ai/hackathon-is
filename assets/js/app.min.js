...
C=c.querySelector("input[name=firstName]"),D=c.querySelector("input[name=lastName]"),z=c.querySelector("input[name=email]"),E=c.querySelector("input[name=phone]"),J=c.querySelector("input[name=agreePartOfTeam]"),F=c.querySelector("input[name=teamName]"),u=c.querySelector(".team-container"),G=c.querySelector("button[name=teamMemberAdd]"),K=c.querySelector("input#challenge1"),L=c.querySelector("input#challenge2"),M=c.querySelector("input[name=agreeTerms]");r&&r.addEventListener("click",k=>{A.classList.add("hide");
B.classList.remove("hide")});w&&w.addEventListener("click",k=>{A.classList.remove("hide");B.classList.add("hide")});u&&G&&G.addEventListener("click",k=>{k.preventDefault();k.stopPropagation();k=u.querySelectorAll(".teamMember").length;if(6>k){let x=Date.now().toString();document.createDocumentFragment();let q=document.createElement("fieldset");q.classList.add("teamMember","form-group","form-group-inline","cols-5-5-2");q.setAttribute("data-id",x);let H=document.createElement("div");var e=document.createElement("input");
e.classList.add("form-control","form-control-2pbb");e.id=`teamMemberName.${k}`;e.name=`teamMemberName.${k}`;e.type="text";e.placeholder="Enter team member name";H.appendChild(e);e=document.createElement("div");var f=document.createElement("input");f.classList.add("form-control","form-control-2pbb");f.id=`teamMemberEmail.${k}`;f.name=`teamMemberEmail.${k}`;f.type="text";f.placeholder="Enter team member e-mail address";e.appendChild(f);f=document.createElement("div");let v=document.createElement("button");
v.classList.add("btn","btn-sm","btn-remove-entry");v.id=`teamMemberDel.${k}`;v.name=`teamMemberDel.${k}`;v.setAttribute("data-id",x);v.innerHTML='<span data-icon="&#xe005;" class="ic-mr-5"></span> Remove';f.appendChild(v);q.appendChild(H);q.appendChild(e);q.appendChild(f);u.appendChild(q);u.querySelectorAll("button.btn-remove-entry").forEach(I=>{I.addEventListener("click",y=>{y=I.getAttribute("data-id");y=u.querySelector(`.teamMember[data-id="${y}"]`);y.parentNode.removeChild(y)})})}});const l={type:null,
...
if(0===q.length||0===f.length)throw"Team member name and e-mail address are mandatory.";if(f.match(b))l.team.members.push({name:q,email:f});else throw`Team member e-mail address for <strong>${q}</strong> is invalid. Please use a different one.`;}})}l.challenge=K.checked?1:L.checked?2:0;if(0===l.challenge)throw"You haven't chosen the hackathon challenge.";l.termsConditions=M.checked?1:0;if(0===l.termsConditions)throw"Terms and Conditions must be agreed in order to complete hackathon registration.";
